...
| 1 | **The Brain** 🧠 | self-aware context (your notes/files) | ✅ `context.py` ingest→retrieve, `/learn` `/ask`, grounded + scored (Ahaan-maths demo); ✅ **cross-session recall** — FTS5 index over past sessions (task + answer), `/recall` (read-only, optional model synthesis), stdlib-only |
...
| 3 | **Easy install + UI** | "click a button"; non-technical install | ✅ `install.sh` one-command (local venv, no sudo) + `webui.py` minimal local web UI (`agent-os ui`, stdlib, localhost) over the same governed router + **`agent-os setup`** guided onboarding (`onboarding.py`: detect → pull → persist choice in `~/.agent-os/config.json`; only `--run` acts, and never installs Ollama itself) + UI first-time empty-state with a **one-click "Pull recommended model"** button (no terminal — `/api/setup` → `run_onboarding()`, the only execution path outside the CLI, reached only by an explicit button click) + **double-click launchers** (`launchers/` macOS `.command` / Windows `.bat` / Linux `.sh`) that bootstrap the env via `install.sh` then open the UI — see [docs/no-terminal.md](no-terminal.md) (signed single-file app needs your dev cert → out of scope) |
| 4 | **Pro-coder + connectors** | augment coders; ingest more sources | 🚧 in progress — ✅ open `SKILL.md` compatibility (recursive/multi-root import via `AGENT_OS_SKILLS_PATH`, model-agnostic injection); ✅ **MCP connector bridge** (`mcp.py`, stdlib stdio JSON-RPC, your own servers in `~/.agent-os/mcp.json`, every call risk-gated + traced + scored + audited; `/mcp`, `/mcp-tools`, `/mcp-call` — [deep dive](mcp.md)); ✅ **closed learning loop** (`skill_synth.py`: a successful, novel, non-trivial job is drafted into a `SKILL.md` and **gated** — written to `skills/` only on `/approve`, never auto-installed); ✅ **role packs** (`packs.py` + bundled `productivity`/`dev` packs: curated `SKILL.md` bundles + credential-free `mcp.example.json`; `/packs` `/pack` `/pack-install`); next: knowledge-graph import, coding-agent links |
